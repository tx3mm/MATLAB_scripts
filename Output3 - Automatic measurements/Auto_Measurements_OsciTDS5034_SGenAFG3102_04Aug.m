...
%%    
    ...
    for freqLoopIdx = 1:length(freqValues)
        ...
        if (freqValues(freqLoopIdx) == 1) &&(timeBase ~= 8.0E-8)
            set(osciDeviceObj.Acquisition(1), 'Timebase', 80.0E-9);
            fprintf(interfaceObj, 'HOR:RECO 50000');
        elseif (freqValues(freqLoopIdx) == 2) && (timeBase ~= 4.0E-8)
            set(osciDeviceObj.Acquisition(1), 'Timebase', 40.0E-9);
        elseif (freqValues(freqLoopIdx) == 10) && (timeBase ~= 2.0E-8)
            set(osciDeviceObj.Acquisition(1), 'Timebase', 20.0E-9);
        end        
