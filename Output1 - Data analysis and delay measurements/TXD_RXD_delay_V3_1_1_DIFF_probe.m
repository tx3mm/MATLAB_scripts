...
%     RXD_data = load([filename '_RXD.mat']);
    ...
    amplValueUsed = split(newstr(3), 'VIO');
...
%     plot(TXD_data.Time, RXD_data.Amplitude,'-m');
    ...
    id = (DIFF_data.Amplitude(startTimeID(1):(end)) >= (0.5*amplValueUsed));
    id_DIFF_rising = find(id) + startTimeID(1)-1;
    
    id = (DIFF_CANH_CANL_data.Amplitude(startTimeID(1):(end)) >= (0.5*amplValueUsed));
