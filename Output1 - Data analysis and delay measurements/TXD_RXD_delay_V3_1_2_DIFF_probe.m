...
% TODO: Add scaling factor prompt in the begining
...
scalingFactor = input('Enter scaling factor: ');
...
%     RXD_data = load([filename '_RXD.mat']);
    ...
    amplValueUsed = split(newstr(4), 'VIO');
...
%         DIFF_data.Amplitude = (CANH_data.Amplitude - CANL_data.Amplitude);
    ...
    DIFF_data.Amplitude = DIFF_data.Amplitude/scalingFactor;
    DIFF_data.Amplitude = DIFF_data.Amplitude - DIFF_data.Amplitude(1);
...
%     plot(TXD_data.Time, RXD_data.Amplitude,'-m');
    plot(DIFF_CANH_CANL_data.Time, DIFF_CANH_CANL_data.Amplitude, 'm');
    ...
    lgd = legend('TXD', 'DIFF_probe', 'CANH-CANL');
    lgd.NumColumns = 1;
    ...
    %% Delay of DIFF_probe from CANH - CANL
    id = (DIFF_data.Amplitude(startTimeID(1):(end)) >= (0.5*max(DIFF_data.Amplitude)));
    id_DIFF_rising = find(id) + startTimeID(1)-1;
    
    id = (DIFF_CANH_CANL_data.Amplitude(startTimeID(1):(end)) >= (0.5*max(DIFF_CANH_CANL_data.Amplitude)));
    id_DIFF_CANH_CANL_rising = find(id) + startTimeID(1)-1;
    
    DIFF_probe_CANH_CANL_delay = DIFF_data.Time(id_DIFF_rising(1))  - DIFF_CANH_CANL_data.Time(id_DIFF_CANH_CANL_rising(1));
...
%     text (2,3, sprintf('BUSREC_RXDH = %d', BUSREC_RXDH));
    ...
    dim = zeros(numberMeasurements,4); %6,4
    str = {['TXD\_busdom = ',num2str(TXD_busdom*1e9) 'ns'];...
        ['TXD\_busrec = ',num2str(TXD_busrec*1e9) 'ns'];...
        ['DIFF\_probe\_CANH\_CANL\_delay = ',num2str(DIFF_probe_CANH_CANL_delay*1e9) 'ns']
...
%     
    ...
    savefig(gcf,[filename '_Figure.fig'])
    filename = [filename '_delays_Plot'];
    print('-f1',filename,'-dpng', '-r300'); %300dpi
    
    close all
    clear TXD_data DIFF_data %RXD_data
